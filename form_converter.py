...
#import google form
prefs = pd.read_csv('/Users/griffinj/Documents/ThesisAssignments/Thesis Advisor Preferences (Responses) - Form Responses 2.csv')

# rename
new_col_names = {'Student Name':'Name',
                 'Student ID':'ID', 
                 'What is your major?':'Major',
                 '(Optional) \nPlease check here if you have do not have a preference for which faculty/project you wish to be assigned to.': 'Equal',
                 ...
                 '(Optional) \nPlease check here if you have do not have a preference for which faculty/project you wish to be assigned to..1':'Equal2',
                 ...
                 '(Optional) \nPlease check here if you have do not have a preference for which faculty/project you wish to be assigned to..2': 'Equal3',
                 ...
                 '(Optional) \nPlease check here if you have do not have a preference for which faculty/project you wish to be assigned to..3': 'Equal4',
                 '(Optional) \nIf applicable, please list the co-advisor or off-campus advisor for your second choice project. .2': 'Neuro_Co_1',
                 '(Optional) \nIf applicable, please list the co-advisor or off-campus advisor for your second choice project. .3': 'Neuro_Co_2',
                 '(Optional) \nIf applicable, please list the co-advisor or off-campus advisor for your second choice project. .4': 'Neuro_Co_3',                 
                 ...
                 '(Optional) \nPlease check here if you have do not have a preference for which faculty/project you wish to be assigned to..4': 'Equal5',
...
for index, row in prefs.iterrows():
    if row['Equal'] == 'Y':
        prefs.iloc[index] = row.str.replace('1', '3').str.replace('2', '3')
    if row['Equal2'] == 'Y':
        prefs.iloc[index] = row.str.replace('1', '3').str.replace('2', '3')   
    if row['Equal3'] == 'Y':
        prefs.iloc[index] = row.str.replace('1', '3').str.replace('2', '3')
...
non_major_columns = ['Name', 'ID', 'Major']

# put all the paragraph statements into separate data frame with just ID
temp_desc =  prefs[prefs.columns[prefs.columns.str.contains('_|Name|ID|Major')]]
...
description.to_csv('/Users/griffinj/Documents/ThesisAssignments/thesis_descrip.csv', index=False)
...
# add BMB, Neuro and inter together and collapse by professor
replacements = {'Bio ': '',
                'Chem ': '',
                'Psych ': '',
                'Neuro ': '',
...
# combin major profs (chem, bio, psych) together
major_profs = pd.concat([major_dfs['Bio'], major_dfs['Chem'], major_dfs['Psych']], ignore_index=True, sort=False)
major_profs = major_profs.dropna(axis = 1, how = 'all')
   
############################
# write files
major_profs.to_csv('/Users/griffinj/Documents/ThesisAssignments/major_profs.csv', index=False)
shared_profs.to_csv('/Users/griffinj/Documents/ThesisAssignments/shared_profs.csv', index=False)
